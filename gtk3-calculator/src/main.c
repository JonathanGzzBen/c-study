...
int main(int argc, char *argv[]) {
  ...
  GtkTextView *operation_text_view = GTK_TEXT_VIEW(gtk_builder_get_object(builder, "operation"));
  operation_text_buffer = gtk_text_view_get_buffer(operation_text_view);
  gtk_text_buffer_set_text(operation_text_buffer, "000000", -1);

  GtkTextView*  prev_operation_text_view = GTK_TEXT_VIEW(gtk_builder_get_object(builder, "prev-operation"));
  prev_operation_text_buffer = gtk_text_view_get_buffer(prev_operation_text_view);
